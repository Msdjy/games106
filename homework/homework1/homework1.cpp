			...
			for (size_t i = 0; i < mesh.primitives.size(); i++) {
				...
				// Vertices
				{
					...
					for (size_t v = 0; v < vertexCount; v++) {
						...
						vert.tangent = tangentsBuffer ? glm::make_vec4(&tangentsBuffer[v * 3]) : glm::vec4(0.0f);
				...
				// Read sampler keyframe output translate/rotate/scale values
				{
					...
					switch (accessor.type)
					{
						case TINYGLTF_TYPE_VEC3: {
							...
							for (size_t index = 0; index < accessor.count; index++)
							{
								dstSampler.outputs.push_back(glm::vec4(buf[index], 0.0f));
							}
							break;
						}
						case TINYGLTF_TYPE_VEC4: {
							...
							for (size_t index = 0; index < accessor.count; index++)
							{
								dstSampler.outputs.push_back(buf[index]);
							}
							break;
						}
						default: {
	...
	void setupDescriptors()
	{
		...
		*/
		...
		std::vector<VkDescriptorPoolSize> poolSizes = {
			...
			vks::initializers::descriptorPoolSize(VK_DESCRIPTOR_TYPE_COMBINED_IMAGE_SAMPLER, static_cast<uint32_t>(static_cast<uint32_t>(5 * glTFModel.materials.size()))),
		};
		...
		// Descriptor set layout for passing material texture
